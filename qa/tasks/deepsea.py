...
class DeepSea(Task):
    ...
    def __init__(self, ctx, config):
        ...
        check_config_key(self.config, 'install', 'source')
        ...
        if self.config['deploy']:
            if not isinstance(self.config['deploy'], dict):
                raise ConfigError("deepsea task: deploy config param takes a dict")
    ...
    def _master_python_version(self, py_version):
        """
        Determine which python version(s) are installed on the Salt Master node.
        Bail out if Python 3 not installed.
        """
    ...
    def _introspect_nodes(self):
        ...
        if self.role_nodes == self.salt_nodes:
            ...
            raise ConfigError("deepsea task: Role nodes ({}) is NOT equal to Salt nodes ({})"
                              .format(self.role_nodes, self.salt_nodes))
    ...
    def _install_deepsea(self):
        """
        Install DeepSea on master node
        """
