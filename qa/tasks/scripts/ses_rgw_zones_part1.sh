...
radosgw-admin zone create --rgw-zonegroup=us --rgw-zone=us-east-1 --endpoints=http://${random_minion_fqdn}:80 \
...
radosgw-admin zone create --rgw-zonegroup=us --rgw-zone=us-east-2 --access-key=$SYSTEM_ACCESS_KEY \
    --secret=$SYSTEM_SECRET_KEY --endpoints=http://${random_minion2_fqdn}:80
radosgw-admin period update --commit
