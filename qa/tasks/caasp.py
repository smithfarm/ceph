...
class Caasp(Task):
    ...
    def __create_cluster(self):
        ...
        self.mgmt_remote.sh("skuba cluster init --control-plane {} cluster".format(remote.hostname))
        self.mgmt_remote.sh("export KUBECONFIG=/home/ubuntu/testcluster/admin.conf")
        self.mgmt_remote.sh("skuba node bootstrap --user ubuntu --sudo --target {} my-master".format(remote.hostname))
        for i in range(4):
            worker_remote = get_remote_for_role(self.ctx, "caasp_worker."+ str(i))
            self.mgmt_remote.sh("skuba node join --role worker --user ubuntu --sudo --target {} worker.{}".format(worker_remote.hostname, str(i)))
