...
munged_options=$(getopt -o c:dhsv --long "cherry-pick-only,component:,debug,existing-pr:,force,help,milestones,prepare,setup,troubleshooting,update-version,usage,verbose,version" -n "$this_script" -- "$@")
...
if [ "$TRACKER_PHASE" ] ; then
    ...
    if [ "$tracker_was_updated" ] ; then
        info "Tracker ${redmine_url} was updated: adding comment"
        data_binary="{\"issue\":{\"notes\":\"ceph-backport.sh version ${SCRIPT_VERSION}: attempting to link this Backport tracker issue with GitHub PR ${desc_should_be}\"}}"
        set -x
        curl --write-out '%{http_code}' --output /dev/null --silent -X PUT --header "Content-type: application/json" --data-binary "${data_binary}" "${redmine_url}.json?key=$redmine_key"
        set +x
